(() => {
	...
	const fetchAndRenderTalks = (container) => {
		const cachedItems = readCacheItems();
		...
		if (
			...
			renderTalksList(cachedItems, container);
		}
	...
	const run = () => {
		...
		const renderWhenReady = (retry = 6) => {
			...
			if (retry <= 0) return;
			window.setTimeout(() => renderWhenReady(retry - 1), 80);
		};
