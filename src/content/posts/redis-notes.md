...
1.运行在内存中：绝大部分请求是纯粹的内存操作，非常快速；  cpu>内存>磁盘

2.redis是运行线程是单线程。采用单线程，避免了不必要的上下文切换和竞争条件，也不存在多进程或者多线程导致的切换而消耗 CPU，不用去考虑各种锁的问题，不存在加锁释放锁操作，没有因为可能出现死锁而导致的性能消耗；
...
>例如，SET lock_key value PX 10000 NX 会将名为 lock_key 的锁设置为在 10 秒后过期。
